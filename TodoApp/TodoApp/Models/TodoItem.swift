...
//        guard let parsedData = try? JSONSerialization.jsonObject(with: json, options: [.mutableContainers, .mutableLeaves]) as? [String: Any]
//        else {
//            print("ошибка распаковки JSON")
//            return nil
//        }
        guard
        ...
        if !(((json["priority"] as? String)?.isEmpty) == nil) {
            var priorityString = json["priority"] as! String
            priority = Priority(rawValue: priorityString)!
            
        }
        let todoData = TodoItem(id: id, text: text, priority: priority, deadline: deadline, done: done , whenCreated: whenCreated , whenEdited: whenEdited )
        return todoData
    }
}
...
extension TodoItem {
    ...
    static func parse(csv: String) -> TodoItem? {
        ...
        let todoData = TodoItem(id: todoString[0], text: todoString[1], priority: priorityString, deadline: Int(todoString[4]), done: done, whenCreated: Int(todoString[6])!, whenEdited: whenEdited ?? nil )
            return todoData
    }
}
//
